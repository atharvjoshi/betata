"""Generate figures of the best T2E traces for each qubit"""
...
if __name__ == "__main__":
    ...
    # dict mapping qubit names and best t2e trace ids
    qubit_traceid_dict = {}
    ...
    for filepath in qubit_metadata_folder.iterdir():
        ...
        with h5py.File(filepath) as file:
            ...
            all_t2es = file["t2e"]["t2e"][:]
            all_t2e_trace_ids = file["t2e"]["t2e_trace_id"][:]
            qubit_traceid_dict[qubit_name] = all_t2e_trace_ids[np.argmax(all_t2es)]

    # retrieve best T2E trace data and generate best T2E figure for each qubit
    for qubit_name, trace_id in qubit_traceid_dict.items():
        if qubit_name not in QUBITS_TO_INCLUDE:
            continue
    
        qubit_trace_data_folder = trace_data_folder / f"{qubit_name}/T2E_{qubit_name}"

        figsavepath = qubit_metadata_folder / f"{qubit_name}_T2E_max.png"
        ...
        best_fit = t2e_fit_fn(tau_ms_dummy, trace.A, T2E_us * 1e-3, 0)
