...
if __name__ == "__main__":
    ...
    for idx, qubit_name in enumerate(QUBITS_TO_INCLUDE):
        ...
        figsavepath = qubit_folder / f"{qubit_name}_T1_T2E_vs_time.svg"
        ...
        omega_t1_million = omega_t1 * 1e-6
        omega_t1_err_million = omega_t1_err * 1e-6

        omega_t2e = omega_q * qubit.t2e
        omega_t2e_err = omega_q * qubit.t2e_err
        t2e_timestamp_day = qubit.t2e_timestamp / 86400

        omega_t2e_million = omega_t2e * 1e-6
        omega_t2e_err_million = omega_t2e_err * 1e-6
        ...
        sel_idxs = rng.choice(len(t1_timestamp_day), size=NUM_SAMPLES, replace=False)
        ...
        y_min = min(min(t1_us), min(t2e_us)) - YLIM_ADJUST[idx]
        y_max = max(max(t1_us), max(t2e_us)) + YLIM_ADJUST[idx]
