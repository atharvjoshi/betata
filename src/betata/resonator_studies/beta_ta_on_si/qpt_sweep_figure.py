"""Generate figure for a representative Qi vs power, temp sweep"""
...
def get_qint_fit(resonator, temps, frs, qints, power, qc, max_temp=None):
    """ """
    num_temps = 101
    if max_temp is None:
        max_temp = max(temps)
    temps_dummy = np.linspace(min(temps), max_temp, num_temps)
...
if __name__ == "__main__":
    ...
    max_temp = None
    resonator_folder = Path(__file__).parents[4] / "out/beta_ta_on_si"
    ...
    for idx, (power, inner_dict) in enumerate(data.items()):
        ...
        temps_dummy, qint_fit = get_qint_fit(
            resonator, temps, frs, qints, power, qc, max_temp=max_temp
        )
    ...
    ax.set_xticks([10, 30, 50, 70, 90, 110], minor=True)
